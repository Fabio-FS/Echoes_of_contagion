...
def plot_trajectory_grid(results_dir, ce_value=0, figsize=(15, 15)):
    """Create 5x5 grid of trajectory plots"""
    thresholds = [-0.8, -0.4, 0, +0.4, 0.8]
    n_bots = [0, 10, 20, 40, 80]
    
    # Load data
    data_grid = load_simulation_data(results_dir, ce_value)
    
    # Create the plot
    fig, axes = plt.subplots(5, 5, figsize=figsize)
    fig.suptitle(f'Opinion Trajectories Grid (Communication Error = {ce_value})', fontsize=16)
    ...
    for i, th in enumerate(thresholds):
        for j, nb in enumerate(n_bots):
            ...
            data = data_grid.get((th, nb))
            
            if data is not None:
                all_results = data['all_results']
                
                # Plot mean opinion trajectories for all replicas
                for replica in all_results:
                    mean_opinions = replica['mean_opinion']
                    time_steps = np.arange(len(mean_opinions))
                    ax.plot(time_steps, mean_opinions, alpha=0.6, linewidth=1)
                
                # Plot median trajectory
                if len(all_results) > 0:
                    mean_opinions_all = np.array([res['mean_opinion'] for res in all_results])
                    median_trajectory = np.median(mean_opinions_all, axis=0)
                    time_steps = np.arange(len(median_trajectory))
                    ax.plot(time_steps, median_trajectory, 'k-', linewidth=2, alpha=0.8)
    ...
    plt.tight_layout()
    return fig, axes, data_grid

def plot_variance_trajectory_grid(results_dir, ce_value=0, figsize=(15, 15)):
    """Same grid but showing opinion variance trajectories"""
    thresholds = [-0.8, -0.4, 0, +0.4, 0.8]
    n_bots = [0, 10, 20, 40, 80]
    ...
    # Create the plot
    fig, axes = plt.subplots(5, 5, figsize=figsize)
    fig.suptitle(f'Opinion Variance Trajectories Grid (Communication Error = {ce_value})', fontsize=16)
    ...
    for i, th in enumerate(thresholds):
        for j, nb in enumerate(n_bots):
            ...
            if data is not None:
                ...
                # Plot variance trajectories for all replicas
                for replica in all_results:
                    opinion_vars = replica['opinion_var']
                    time_steps = np.arange(len(opinion_vars))
                    ax.plot(time_steps, opinion_vars, alpha=0.6, linewidth=1)
                
                # Plot median trajectory
                if len(all_results) > 0:
                    opinion_vars_all = np.array([res['opinion_var'] for res in all_results])
                    median_trajectory = np.median(opinion_vars_all, axis=0)
                    time_steps = np.arange(len(median_trajectory))
                    ax.plot(time_steps, median_trajectory, 'k-', linewidth=2, alpha=0.8)
